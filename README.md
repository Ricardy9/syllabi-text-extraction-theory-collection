...
2. **`metadata_on_syllabi.py`** — extracts course-level metadata from each syllabus PDF (title, professor(s), university, year, term) using pdfplumber for text extraction and the Claude API for intelligent field inference. Writes `metadata_on_syllabi.xlsx` and `metadata_on_syllabi.csv`.
3. **`extract_readings_from_syllabi.py`** — extracts every bibliographic reference from the reading lists in each syllabus using the Claude API, deduplicates entries that appear across multiple syllabi, enriches missing DOIs via Crossref, and writes a formatted Excel workbook with dropdown validation. Also saves per-syllabus JSON files to `extracted_references/`.
4. **`most_popular_readings.py`** *(optional)* — generates an interactive HTML bar chart ranking readings by how many syllabi they appear in. Reads `literature_from_selected_syllabi.xlsx` and writes `most_popular_readings.html`.
5. **`newest_readings.py`** *(optional)* — generates an HTML leaderboard of the most recently published readings, sorted by year (newest first). Reads `literature_from_selected_syllabi.xlsx` and writes `newest_readings.html`.

> **Run order:** Stage 1 → Stage 2 → Stage 3. Stages 4 and 5 can be run any time after Stage 3. Stage 3 reads `metadata_on_syllabi.xlsx` produced by Stage 2 to build descriptive class labels.
...
├── metadata_on_syllabi.py         # Stage 2: course metadata extraction
├── extract_readings_from_syllabi.py      # Stage 3: reading-list extraction
├── most_popular_readings.py                 # Stage 4: cross-syllabus frequency leaderboard
├── newest_readings.py         # Stage 5: most-recent readings leaderboard
│
├── text_review/                           # Output of Stage 1
│   ├── <syllabus name>.md                 #   Raw extracted text per syllabus
│   └── index.html                         #   Browser viewer
│
├── metadata_on_syllabi.xlsx                  # Output of Stage 2
├── metadata_on_syllabi.csv                   # Output of Stage 2 (CSV copy)
│
...
│
├── most_popular_readings.html               # Output of Stage 4
└── newest_readings.html       # Output of Stage 5
```
...
```bash
python metadata_on_syllabi.py
```

Reads all PDFs from `Syllabi to Draw From/` and produces `metadata_on_syllabi.xlsx` and `metadata_on_syllabi.csv` with the following columns:
...
```bash
python extract_readings_from_syllabi.py
```
...
Source type is classified by Claude during extraction. Class labels in the "Class/es" column are built from `metadata_on_syllabi.xlsx` in the format `"Course Title" Professor(s) – Term Year`; if that file is absent, raw PDF filenames are used.
...
```bash
python most_popular_readings.py
```

Reads `literature_from_selected_syllabi.xlsx` and produces `most_popular_readings.html`: an interactive bar chart ranking readings by how many syllabi they appear in. Only readings that appear in at least `MIN_SYLLABI` syllabi (default: 2) are included. The page also lists all analyzed syllabi with course title, instructors, term, year, and university pulled from `metadata_on_syllabi.xlsx`.

Open `most_popular_readings.html` in a browser — no server required.
...
```bash
python newest_readings.py
```

Reads `literature_from_selected_syllabi.xlsx` and produces `newest_readings.html`: a sortable HTML table of the top `TOP_N` most recently published readings (default: 50), sorted by year (newest first). Each row shows title, authors, year, publication venue, DOI link, and the syllabus/syllabi where the reading was assigned.

Open `newest_readings.html` in a browser — no server required.
...
- The `LLM_MODEL` variable in `metadata_on_syllabi.py` defaults to `claude-haiku-4-5-20251001` for speed and cost. Change it to `claude-sonnet-4-6` for harder cases.
